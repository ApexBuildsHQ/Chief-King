...
def generate_and_upload_hf(recipe, recipe_idx, total_recipes):
    ...
    # استخدام نموذج SDXL المضمون ويدعم الخطة المجانية على الـ Router
    ...
    negative_prompt = "blurry, distorted, ugly, low quality, bad anatomy, text, watermark, signature, oversaturated, unrealistic, plastic look"
    ...
    for attempt in range(3):
        ...
        try:
            log(f"🎨 [إنشاء AI] جاري طلب توليد صورة SDXL جودة عالية للوصفة: '{recipe_name}'")
            log(f"📝 [Prompt]: {prompt[:110]}...")
            ...
            if response.status_code == 200 and len(response.content) > 15000:
                ...
                # رفع الصورة المكتملة إلى Dataset
                ...
                log(f"⚠️ [استجابة غير صالحة] كود الحالة: {response.status_code} | المحتوى: {response.text[:120]}")
