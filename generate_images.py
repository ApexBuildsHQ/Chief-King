...
def fetch_and_upload_image(recipe_name, file_num, recipe_idx, total_recipes):
    ...
    for attempt in range(3):
        ...
        try:
            ...
            headers = {'User-Agent': f'Mozilla/5.0 (Windows NT 10.0; Win64; x64; Bot_File{file_num}_Index{recipe_idx}_Attempt{attempt})'}
            response = requests.get(ai_url, headers=headers, timeout=20)
            
            if response.status_code == 200 and len(response.content) > 3000:
                log(f"📥 [تم التحميل] حجم الصورة: {len(response.content)} bytes")
                ...
                log(f"⚠️ [استجابة غير صالحة] كود الحالة: {response.status_code}")
        except Exception as e:
