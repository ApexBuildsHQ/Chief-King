...
# تهيئة عملاء Hugging Face الرسميين
...
def generate_ai_prompt(recipe):
    ...
    clean_desc = description[:120] if description else "freshly cooked appetizing meal"
    ...
    prompt = (
        f"Professional award-winning studio food photography of {name}. "
        f"{clean_desc}. Category: {category}{kw_str}. "
        f"Gourmet plating, soft culinary lighting, 8k resolution, highly detailed texture, DSLR photo, appetizing presentation, close up."
    )
    return prompt
...
def generate_and_upload_hf(recipe, recipe_idx, total_recipes):
    ...
    # قائمة بالنماذج المعتمدة والتلقائية عبر InferenceClient
    models_to_try = [
    ...
    for attempt, model_name in enumerate(models_to_try):
        try:
            log(f"🎨 [إنشاء AI] طلب توليد صورة بالنموذج '{model_name}' للوجبة ({recipe_idx + 1}/{total_recipes}): '{recipe_name}'")
            log(f"📝 [Prompt]: {prompt[:100]}...")
            
            # استدعاء مباشر عبر العميل الرسمى للتغلب على مشاكل العناوين والـ Endpoints
            image = client.text_to_image(
                prompt=prompt,
                model=model_name
            )
            
            # حفظ الصورة مؤقتاً بصيغة JPEG
            if image.mode != "RGB":
                image = image.convert("RGB")
            image.save(image_filename, "JPEG", quality=90)
            ...
            log(f"⚠️ [محاولة {attempt + 1} فشلت مع {model_name}]: {e}")
            time.sleep(3)

    log(f"🛑 [فشل كامل] تعذر التوليد بعد تجربة كافة النماذج للوجبة: '{recipe_name}'.")
    return ""
...
def run_pipeline():
    ...
    for idx, recipe in enumerate(recipes):
        ...
        # التثبت مما إذا كانت الصورة محددة ومرفوعة سابقاً
    ...
    log(f"\n🎉 [اكتمال السيرفر #{FILE_NUM}] تم الانتهاء من جميع الوصفات الخاصة بهذا السيرفر بنجاح!")
