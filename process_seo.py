...
1. 'name': Most searched, natural English recipe title.
   ...
   - NEVER change the fundamentally correct dish type (e.g., NEVER convert a sandwich, soup, or pie into a 'Lasagna' or 'Cake').
   - NEVER drop essential main ingredients, dietary types, or styles (e.g., KEEP 'Vegetable', 'Chicken', 'Vegan', 'Thai', 'Cajun' if they exist in the original title).
   - NEVER add new main ingredients that are not in the original title (e.g., DO NOT add chicken to a dessert/caramel corn).
   - KEEP unique seasonal, shaped, or themed recipe names (e.g., 'Pizza Skulls') while only removing technical/junk text.
   - If the original title is already clean, natural, and optimal (e.g., 'Fluffy White Cake'), keep it EXACTLY as it is.
